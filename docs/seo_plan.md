...
> ⚠️ **方針確定（2026-07-08）：中国本土は当面対象外。以下 §8.1–8.3 は本土本格参入を判断した場合のバックログ。** 現行は §8.0 の決定どおり香港/台湾/海外華人にフォーカス（追加コード不要）。

背景：訪日外国人で**中国は最大級の市場**、zh-CN(簡体字)も実装済み。ただし中国は**検索・AI・インフラが西側と完全に別物**。§7（Google/ChatGPT系）はそのままでは中国本土に効かない。

## 8.0 🔴 最重要の前提：本土アクセス性 → ✅ **方針確定（2026-07-08）：中国本土は対象外**

**決定：香港・台湾・海外華人にフォーカスし、中国本土は当面対象外とする。**（下記 選択肢2）

理由（実測）：現スタックは **Google依存が深い**（App Check=reCAPTCHA Enterprise を購入含む6関数で強制／Firebase Auth・Google ログイン／Firestore/Storage=googleapis／アセット=storage.googleapis.com）。これらは**中国本土で遮断/低速** → 本土ユーザーは「開けない/ログイン不可/購入ブロック」。一方 **香港・台湾は非遮断**で現状のまま動作し、**zh-TW(繁体字)実装済み・Stripe決済可・ICP/中国CDN不要**。

**この決定の含意（＝やること/やらないこと）**：
- ✅ **香港/台湾/海外華人**：既存実装で対応済み（zh-TW＋hreflang `zh-Hant`/`zh-TW`＋プリレンダ済）。追加コード不要。Google/Bing で獲得。
- ✅ **zh-CN(簡体字)は残す**：シンガポール/マレーシア華人＋「旅行中で本土外にいる中国人」に有効（無害）。ただし百度向けGEO・小红书等の本土オフページ施策は**追わない**。
- ❌ **やらない**：ICP备案／中国CDN／App Check中国代替／百度站长登録／中国クローラ最適化（§8.1–8.3 は本土本格参入時のバックログとして保留）。
- 🔁 **再判断の条件**：本土トラフィック/需要が計測で顕在化したら選択肢1（本格参入＝別プロジェクト級）を検討。

（参考・保留した選択肢）
1. **本格参入**：中国国内CDN ＋ **ICP备案**（中国法人/パートナー必須）＋ App Check/認証の中国代替 ＋アセット中国配信 ＝ **別プロジェクト級**（本土需要の計測後に判断）
2. ✅**採用＝割り切り**：本土対象外・香港/台湾/海外華人にフォーカス（zh-TW＝非遮断）
3. 段階移行（1へ）
